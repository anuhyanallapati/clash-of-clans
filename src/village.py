...
class Village:
    def __init__(self, config, level, King):
        ...
        self.map = self.generate_map(King)             # map of the village

    def generate_map(self, King):
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall, self, self.level, King)
        ...
        for cannon in self.cannons:                                     # cannon is 2x2
            cannon_obj = bd.Cannon(cannon, self, self.level)
        ...
        for wizard_tower in self.wizard_towers:                          # wizard tower is 1X1
            wizard_tower_obj = bd.WizardTower(wizard_tower, self, self.level)
